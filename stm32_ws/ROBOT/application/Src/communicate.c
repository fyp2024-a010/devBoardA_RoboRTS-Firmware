...
//  osSignalSet(communicate_task_t, RECV_PROTOCOL_SIGNAL);
  osThreadFlagsSet(communicate_task_t, RECV_PROTOCOL_SIGNAL);
  return len;
}
...
//    osEvent event;
	uint32_t event;
    event = osThreadFlagsWait(SEND_PROTOCOL_SIGNAL | RECV_PROTOCOL_SIGNAL, osFlagsWaitAny, osWaitForever);

	if (event & SEND_PROTOCOL_SIGNAL)
	{
	protocol_send_flush();
	}

	if (event & RECV_PROTOCOL_SIGNAL)
	{
	protocol_unpack_flush();
	}

//    event = osSignalWait(SEND_PROTOCOL_SIGNAL | RECV_PROTOCOL_SIGNAL, osWaitForever); // CMSIS_v1
...
static void protocol_send_success_callback(void)
{
//  osSignalSet(communicate_task_t, SEND_PROTOCOL_SIGNAL); // CMSIS_v1
	osThreadFlagsSet(communicate_task_t, SEND_PROTOCOL_SIGNAL);
}
